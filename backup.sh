...
mkdir $fecha
cd /home/marcosguz/Escritorio/Seguridad
vacio= $(ls /var/tmp/Backups/SGSSI/Sec/$fechaAnt | grep “$fechaAnt”)
if [ -z $vacio ] then
	rsync -av /home/marcosguz/Seguridad/ /var/tmp/Backups/SGSSI/Sec/$fecha
else
rsync -av --link-dest=/var/tmp/Backups/SGSSI/Sec/$fechaAnt . /var/tmp/Backups/SGSSI/Sec/$fecha
fi
cd /var/tmp/Backups/SGSSI/BD
mkdir $fecha
mysqldump -u 'marcosguz' -p '1384001u.D' 'SGSSI' > /var/tmp/Backups/SGSSI/BD/$fecha/backup_SGSSI.sql #crear backup
#              username        db_name
...
#para hacerlo cada dia a las 12 del mediodia: 0 12 * * * /home/marcosguz/script_folder/backup.sh
#*****:
